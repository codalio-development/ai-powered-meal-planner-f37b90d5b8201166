...
export default function RecommendedRecipes() {
  ...
  return (
   <div className="bg-[var(--color-background)] text-[var(--color-text)]">
      ...
      <Card className="rounded-2xl bg-[var(--color-primary)]/15 border border-[var(--color-primary)]/30">
      <CardBody className="p-6 flex items-center justify-between gap-4">
      <div>
      <h2 className="text-lg font-semibold text-[var(--color-text)]">Tailored to you</h2>
      <p className="text-sm text-[var(--color-text)]/80">Based on your favorites and preferences</p>
      </div>
      <div className="flex gap-2">
      <Chip size="sm" className="rounded-full bg-[var(--color-primary)] text-[var(--color-text)]">Vegetarian</Chip>
      <Chip size="sm" className="rounded-full bg-[var(--color-secondary)] text-[var(--color-text)]">Mediterranean</Chip>
      </div>
      </CardBody>
      </Card>
      </section>
